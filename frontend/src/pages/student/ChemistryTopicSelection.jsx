...
import { 
  ...
  TOPICS_BY_GRADE, 
  DIFFICULTIES 
} from '../../data/chemistryExperiments';

export default function ChemistryTopicSelection() {
  ...
  const filteredExperiments = useMemo(() => {
    return CHEMISTRY_EXPERIMENTS.filter(exp => {
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <button 
            onClick={() => navigate('/student/lab/chemistry')}
            className="p-2 hover:bg-gray-100 rounded-full transition-colors"
          >
            <ArrowLeft className="w-6 h-6 text-gray-600" />
          </button>
          <div>
            <h1 className="text-3xl font-bold text-gray-900">{formattedGrade} Chemistry Library</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredExperiments.map((exp) => (
          <ExperimentCard 
            key={exp.id} 
            experiment={exp} 
            onStart={() => navigate(`/student/lab/chemistry/experiment/${exp.id}`)}
            onPreview={() => navigate(`/student/lab/chemistry/experiment/${exp.id}`)}
          />
        ))}
      ...
      <div className="pt-8 flex flex-col md:flex-row items-center justify-between border-t border-gray-100 gap-4">
        <p className="text-xs text-gray-400 font-medium">Virtual Lab • Chemistry • {formattedGrade} • Experiment Library</p>
        <div className="flex items-center space-x-4">
