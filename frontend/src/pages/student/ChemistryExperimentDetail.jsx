...
import { getExperimentById } from '../../data/chemistryExperiments';

export default function ChemistryExperimentDetail() {
  const { id } = useParams();
              ...
              <div className="space-y-8 animate-in fade-in duration-300">
                <div className="text-center space-y-4 py-12">
                   ...
                   <div className="mt-8 space-y-6 text-left">
                     {experiment.quiz && experiment.quiz.map((qItem, idx) => (
                       <div key={idx} className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                          <p className="font-bold text-gray-800 mb-4">{idx + 1}. {qItem.q}</p>
                          <div className="space-y-2">
                             {qItem.options.map((opt, optIdx) => (
                               <label key={optIdx} className="flex items-center space-x-3 p-3 bg-white border border-gray-200 rounded-xl cursor-pointer hover:border-primary-vibrant/50 transition-colors">
                                  <input type="radio" name={`q-${idx}`} className="text-primary-vibrant focus:ring-primary-vibrant" />
                   ...
                   <button className="px-8 py-3 bg-primary-vibrant text-white rounded-xl font-bold shadow-lg hover:shadow-primary-vibrant/40 transition-all mt-6">
                      Submit Answers
                   </button>
                </div>
              </div>
