...
function Scene({ mass, springK, topY }) {
  ...
  const bottomY = topY - 2 - extension;
...
export default function HookesLaw3D() {
  ...
  const theory = {
    formula: 'F = kx  →  x = F/k = mg/k',
    values: [
      { label: 'Verification (F/x)', value: `${(force / Math.max(extension, 0.001)).toFixed(1)} N/m ≈ k` },
      { label: 'Parameters', value: `m = ${mass.toFixed(1)}kg, g = 9.81 m/s², k = ${springK} N/m` },
    ],
  };

  return (
    <ExperimentShell title="Hooke's Law" controls={controls} theory={theory}>
