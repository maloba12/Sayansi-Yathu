...
export default function LinearMotion3D() {
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
        ...
        <button onClick={() => { setIsRunning(false); setLiveData({ displacement: 0, velocity: initialVelocity, time: 0 }); }}
          style={{
            padding: '10px',
            background: '#dd6b20', color: '#fff', border: 'none', borderRadius: '6px',
            cursor: 'pointer', fontWeight: 'bold',
          }}>
          🔄 Reset
        </button>
  ...
  const theory = {
    formula: 's = v₀t + ½at²,  v = v₀ + at',
    values: [
      { label: 'Parameters', value: `v₀ = ${initialVelocity.toFixed(1)} m/s, a = ${acceleration.toFixed(1)} m/s²` },
    ],
  };

  return (
    <ExperimentShell title="Linear Motion" controls={controls} theory={theory}>
